...
- `source_row_id`: liên kết dòng `asset_source_rows` gốc nếu tài sản đến từ đối chiếu bàn giao. **Không thêm bất kỳ cột nào lên bảng `asset_source_rows`** — trạng thái "đã đối chiếu bao nhiêu" luôn được tính bằng `SUM(quantity)` (bỏ qua `NULL`) trên các `assets` trỏ tới dòng đó, không lưu cache. Dùng `SUM(quantity)` chứ không phải `COUNT(*)` vì với loại đếm số lượng (`durable_goods`/`infrastructure`), 1 dòng `assets` có thể đại diện nhiều đơn vị (ví dụ 1 dòng `quantity=14` cho 14 cái giường) — đếm số dòng sẽ sai; với loại cá thể, `quantity` luôn `=1` nên `SUM` cho kết quả giống hệt đếm số dòng.
...
### Đối chiếu bàn giao
- `GET /api/asset-source-rows?documentId=X` (đã có, Giai đoạn 2) — **mở rộng thêm field** `reconciledCount` (số nguyên = `SUM(quantity)` trên các `assets` có `source_row_id = row.id`, bỏ qua dòng `quantity IS NULL`, mặc định `0` nếu chưa có dòng nào) vào mỗi dòng trả về. Không đổi role/tham số hiện có.
